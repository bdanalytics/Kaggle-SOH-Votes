...
title: "Show of Hands:Votes: Party classification:: Votes_Q124742"
author: "bdanalytics"
...
glbFeatsDerive[["Hhold.fctr"]] <- list(
    mapfn = function(HouseholdStatus) {
        raw <- HouseholdStatus
        raw[raw %in% ""] <- "N"
...
glbFeatsDerive[["Edn.fctr"]] <- list(
    mapfn = function(EducationLevel) { raw <- EducationLevel;
        raw[raw %in% ""] <- "N"
...
glbFeatsDerive[["Q124742.fctr"]] <- list(
    mapfn = function(Q124742) {
        raw <- Q124742
        raw[raw %in% ""] <- "NA"
        raw <- gsub("Yes", "Ys", raw, fixed = TRUE)
        if (length(setdiff(unique(raw), c("NA", "No", "Ys"))) > 0)
            stop("Q124742 vals: ", unique(raw), 
                 " does not match expectation: ", c("NA", "No", "Ys"))
        return(relevel(as.factor(raw), ref = "NA"))
    }
    , args = c("Q124742"))
...
glbOut <- list(pfx = "Votes_Q124742_")
...
#load("Votes_Q124742_extract.features.end.RData", verbose = TRUE)
