...
title: "Show of Hands:Votes: Party classification:: Q109244Yes_AllX_cnk01_cluster.data"
author: "bdanalytics"
...
glbOut <- list(pfx = "Q109244Yes_AllX_cnk01_cluster.data_")
...
glbChunks[["last" ]] <- "cluster.data" # default: script will save envir at end of this chunk 
glbChunks[["inpFilePathName"]] <- NULL # "data/Q109244Yes_AllX_cnk01_cluster.data_cnk01_manage.missing.data_manage.missing.data.RData" # NULL: default or "data/<prvScriptName>_<lstChunkLbl>.RData"
...
for (mdlId in unique(glbMdlSltId, glbMdlFnlNslId, glbMdlFnlRslId)) {
    ...
    for (clmn in names(glbObsOut$vars)) {
        ...
        if (mdlId == glbMdlFinNslId) feat <- gsub("glbMdlId", "glbMdlFnlNslId", feat) else
        if (mdlId == glbMdlFinRslId) feat <- gsub("glbMdlId", "glbMdlFnlRslId", feat) else
        if (mdlId == glbMdlSltId   ) feat <- gsub("glbMdlId", "glbMdlSltId"   , feat)
        thsObsOutDf[, clmn] <- eval(parse(text = feat))
    }
    if (mdlId == glbMdlFinNslId) obsOutFnlNslDf <- thsObsOutDf else
    if (mdlId == glbMdlFinRslId) obsOutFnlRslDf <- thsObsOutDf else
...
if (!is.null(glbOutStackFnames)) {
    for (fname in glbOutStackFnames) {
        ...
        #obsOutFnlNslDf <- dplyr::arrange_(rbind(obsOutFnlNslDf, read.csv(fname)), "UniqueID")
        obsOutFnlNslDf <- dplyr::arrange_(rbind(obsOutFnlNslDf, 
                #read.csv(fname) %>% filter(!(UniqueID %in% obsOutFnlNslDf$UniqueID))),
            #read.csv(fname) %>% filter(!(UniqueID %in% obsOutFnlNslDf[, glbFeatsId]))),
            read.csv(fname) %>% 
                dplyr::filter_(interp(~!(var %in% obsOutFnlNslDf$var), 
                                      var = as.name(glbFeatsId)))),
                                    glbFeatsId)
        
        if (nrow(obsOutFnlNslDf) != length(unique(obsOutFnlNslDf[, glbFeatsId])))
            stop("Potential dups in stacked prediction output")
        
        obsOutSltDf <- dplyr::arrange_(rbind(obsOutSltDf, 
                read.csv(fname) %>% 
                dplyr::filter_(interp(~!(var %in% obsOutSltDf$var), 
                                      var = as.name(glbFeatsId)))),
                                    glbFeatsId)
        
        if (nrow(obsOutSltDf) != length(unique(obsOutSltDf[, glbFeatsId])))
            stop("Potential dups in stacked prediction output")
    }
}

for (mdlId in unique(glbMdlFnlNslId, glbMdlFnlRslId, glbMdlSltId)) {
    if (mdlId == glbMdlFnlNslId) thsObsOutDf <- obsOutFnlNslDf else
    if (mdlId == glbMdlFnlRslId) thsObsOutDf <- obsOutFnlRslDf else
    if (mdlId == glbMdlSltId)    thsObsOutDf <- obsOutSltDf
    
    if (!is.null(glbObsOut$mapFn)) 
        thsObsOutDf <- glbObsOut$mapFn(thsObsOutDf)
    
    write.csv(thsObsOutDf, sprintf("%sout_%s.csv", glbOut$pfx, mdlId), 
