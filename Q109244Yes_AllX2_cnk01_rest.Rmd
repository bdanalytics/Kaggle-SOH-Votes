...
title: "Show of Hands:Votes: Party classification:: Q109244Yes_AllX_cnk01_rest"
author: "bdanalytics"
...
Data: 
Source: 
    Training:   "https://inclass.kaggle.com/c/can-we-predict-voting-outcomes/download/train2016.csv"  
    New:        "https://inclass.kaggle.com/c/can-we-predict-voting-outcomes/download/test2016.csv"  
Time period: 

```{r set_global_options_wd, echo=FALSE}
setwd("~/Documents/Work/Courses/MIT/Analytics_Edge_15_071x/Assignments/Kaggle_SOH_Votes")
...
glbOut <- list(pfx = "Q109244Yes_AllX2_cnk01_rest_")
...
glbChunks[["inpFilePathName"]] <-  "data/Q109244Yes_AllX2_cnk01_cluster.data_cluster.data.RData" # NULL: default or "data/<prvScriptName>_<lstChunkLbl>.RData"
...
```{r scrub.data, cache=FALSE, echo=FALSE, eval=myevlChunk(glbChunks, glbOut$pfx, keepSpec = c("glbMdlFamilies", "glbMdlTuneParams","glbFeatsCategory","glb_dsp_cols","glbMdlPreprocMethods","glbMdlSltId","glbMdlSequential"), dropSpec = c(NULL))}
#myloadChunk(glbChunks$inpFilePathName, keepSpec = c("glbMdlFamilies", "glbMdlTuneParams","glbFeatsCategory","glb_dsp_cols","glbMdlPreprocMethods","glbMdlSltId","glbMdlSequential"), dropSpec = c(NULL)); glb2Sav()

# glbObsAll$Hhold.fctr <- plyr::revalue(glbObsAll$Hhold.fctr, c(
#         "SKy"  = "my.oth",
#         "PKn"  = "my.oth",        
#         "PKy"  = "my.oth",                
#         "**"   = "**"))
# glbObsTrn <- glbObsTrn[!(glbObsTrn$USER_ID %in% c(4667, 5919)), ]
# glbObsAll <- glbObsAll[!(glbObsAll$USER_ID %in% c(4667, 5919)), ]
...
if (all(is.na(glbObsNew[, glb_rsp_var]))) {
    ...
    lclgetMatrixSimilarity <- function(x, y, weights = NULL) {
        require(proxy)
        ...
        xDf <- foreach(metric = metrics, .combine = cbind) %dopar% {
            if (metric != "mywgtCosine")
                xmDf <- data.frame(method = rowMeans(
                    proxy::simil(x, y, method = metric),
                                                    na.rm = TRUE)) else
                xmDf <- data.frame(method = rowMeans(
                    proxy::simil(x, y, method = metric, weights = weights),
                                                    na.rm = TRUE))
            names(xmDf) <- metric
            xmDf
        }
...
#             as.character(rspvar_freq_df[, glb_rsp_var_raw])
        ...
        TrnNewSmy <- lclgetMatrixSimilarity(mtxObsTrn, mtxObsNew, weights = ftrWgt)
    ...
    OOBFitSmy <- lclgetMatrixSimilarity(mtxObsOOB, mtxObsFit, weights = ftrWgt)
    OOBNewSmy <- lclgetMatrixSimilarity(mtxObsOOB, mtxObsNew, weights = ftrWgt)
    # OOBNewSmy[, glbFeatsId] <- glbObsOOB[, glbFeatsId]
    FitNewSmy <- lclgetMatrixSimilarity(mtxObsFit, mtxObsNew, weights = ftrWgt)
...
```{r fit.models_1, cache=FALSE, echo=FALSE, eval=myevlChunk(glbChunks, glbOut$pfx, keepSpec = c("glbMdlFamilies","glbMdlSequential","glbMdlPreprocMethods","glbMdlTuneParams","glbMdlSltId","glbMdlEnsemble"), dropSpec = c(NULL))}
...
require(stringr)
obsOutDf <- obs_df[, glbFeatsId, FALSE]
# Always output glbMdlFnlNsl
for (mdlId in unique(c(glbMdlSltId, glbMdlFnlNslId, glbMdlFnlRslId))) {
    thsObsOutDf <- obsOutDf
    for (clmn in names(glbObsOut$vars)) {
        #obsOutSltDf[, clmn] <- obs_df[, glbObsOut$vars[[clmn]]] # deprecated        
        stopifnot(grepl("^%<d-%", glbObsOut$vars[[clmn]]))

        feat <- str_trim(unlist(strsplit(glbObsOut$vars[[clmn]], "%<d-%"))[2])
        if (mdlId == glbMdlFnlNslId) feat <- gsub("glbMdlId", "glbMdlFnlNslId", feat) else
        if (mdlId == glbMdlFnlRslId) feat <- gsub("glbMdlId", "glbMdlFnlRslId", feat) else
        if (mdlId == glbMdlSltId   ) feat <- gsub("glbMdlId", "glbMdlSltId"   , feat)
        thsObsOutDf[, clmn] <- eval(parse(text = feat))
    }
    if (mdlId == glbMdlFnlNslId) obsOutFnlNslDf <- thsObsOutDf else
    if (mdlId == glbMdlFnlRslId) obsOutFnlRslDf <- thsObsOutDf else
    if (mdlId == glbMdlSltId   ) obsOutSltDf    <- thsObsOutDf
}
# savOutFnlNslDf <- obsOutFnlNslDf; savOutFnlRslDf <- obsOutFnlRslDf; savOutSltDf <- obsOutSltDf
...
if (glb_is_classification) {
    ...
    if (".grpid" %in% names(glbObsNew)) {
        ...
        tmp_newobs_df <- 
            merge(tmp_newobs_df, obsOutFnlNslDf, by = glbFeatsId, all.x = TRUE)
        tmp_newobs_df$.err <- 
...
#predctId <- mygetPredictIds(glb_rsp_var, glbMdlFnlNslId)$value; errObsNew <- glbObsNew[(glbObsNew[, glbFeatsCategory] %in% tmpLvlCategory[, 1]), ];
#myprint_df(errObsNew[(errObsNew[, glbFeatsCategory] %in% tmpLvlCategory[errLvlIx, glbFeatsCategory]) & (errObsNew[, predctId] == "Y"), union(c(glbFeatsId, glbFeatsCategory, predctId), myextract_actual_feats(row.names(glb_featsimp_df[glb_featsimp_df[, paste0(glbMdlFnlNslId, ".imp")] > 10, ])))])
#myprint_df(errObsNew[(errObsNew[, glbFeatsCategory] %in% "Foreign#World#") & (errObsNew[, predctId] == "Y"), union(c(glbFeatsId, glbFeatsCategory, predctId), myextract_actual_feats(row.names(glb_featsimp_df[glb_featsimp_df[, paste0(glbMdlFnlNslId, ".imp")] > 10, ])))])
    }
...
#stop(here"); glb2Sav(); sav_obsOutFnlNslDf <- obsOutFnlNslDf; all.equal(sav_obsOutFnlNslDf, obsOutFnlNslDf); obsOutFnlNslDf <- sav_obsOutFnlNslDf

# This does not work for classification since AUC distribution might be different for different models
#   -> Run glm on .prob from this glbMdlFnlNslId & .prob from stacked file & stack condition as a feature
if (!is.null(glbOutStackFnames)) {
    # for (fname in glbOutStackFnames) {
    fname <- glbOutStackFnames[1]
    print(sprintf("Stacking file %s to prediction outputs...", fname))
    stackDf <- read.csv(fname)
    
    for (mdlId in unique(c(glbMdlSltId, glbMdlFnlNslId, glbMdlFnlRslId))) {
        if (mdlId == glbMdlFnlNslId) obsOutDf <- obsOutFnlNslDf else
        if (mdlId == glbMdlFnlRslId) obsOutDf <- obsOutFnlRslDf else
        if (mdlId == glbMdlSltId   ) obsOutDf <- obsOutSltDf

        obsOutDf <- stackDf %>% 
            dplyr::filter_(interp(~!(var %in% obsOutDf$var), var = as.name(glbFeatsId))) %>% 
            rbind(obsOutDf) %>%
            arrange_(glbFeatsId)
        stopifnot(nrow(obsOutDf) == length(unique(obsOutDf[, glbFeatsId])))

        if (!is.null(glbObsOut$mapFn)) 
            obsOutDf <- glbObsOut$mapFn(obsOutDf)
    
        write.csv(obsOutDf, sprintf("%sout_%s.csv", glbOut$pfx, mdlId), 
                    quote = FALSE, row.names = FALSE)
        #cat(" ", "\n", file=submit_fn, append=TRUE)

        # does not work due to # in value of mdlId
        # dfSyl <- eval(parse(text = sprintf('switch(mdlId, 
        #                    %s = "obsOutFnlNslDf", # This shd always be output
        #                    %s = "obsOutFnlRslDf",
        #                    %s    = "obsOutSltDf"   ,
        #                    "ThisShouldNotHappen")', 
        #                    glbMdlFnlNslId, glbMdlFnlRslId, glbMdlSltId)))
        # stopifnot(dfSyl != "ThisShouldNotHappen")
        # assign(dfSyl, obsOutDf)
        
        if (mdlId == glbMdlFnlNslId) obsOutFnlNslDf <- obsOutDf else
        if (mdlId == glbMdlFnlRslId) obsOutFnlRslDf <- obsOutDf else
        if (mdlId == glbMdlSltId   ) obsOutSltDf    <- obsOutDf
    }
    
    # all.equal(obsOutFnlNslDf %>% dplyr::filter_(interp(~(var %in% savOutFnlNslDf$var), var = as.name(glbFeatsId))), savOutFnlNslDf)    
    # all.equal(obsOutFnlNslDf %>% dplyr::filter_(interp(~!(var %in% savOutFnlNslDf$var), var = as.name(glbFeatsId))), stackDf %>% dplyr::filter_(interp(~!(var %in% savOutFnlNslDf$var), var = as.name(glbFeatsId))))    
}
...
if (!is.null(glbFeatsCategory)) {
    ...
    clmnOrder[, "prefer"] <- sapply(gsub("(.+)(Fit|fit|OOB|trn|new|Tst)\\.sum", "\\2",
                                         clmnOrder$metric),
    ...
    if (glb_is_regression) {    
        pred_stats_df <- 
            mypredict_mdl(mdl = glb_models_lst[[glbMdlFnlNslId]], 
                          ...
                          rsp_var = glb_rsp_var, 
                          label = "new",
                model_summaryFunction = glbMdlSlt$control$summaryFunction,
                          model_metric = glbMdlSlt$metric,
                          model_metric_maximize = glbMdlSlt$maximize,
                          ret_type = "stats")        
